...
- `fatou.schema.json` (generated by
...
- documentation oracle `expected.sexpr` projections and `.markdown-source`
  metadata under `crates/fatou-parser/tests/fixtures/documentation/oracle/`
  (generated by `scripts/update-markdown-corpus.sh`)
...
The `documentation` module separately parses decoded Julia Markdown into a
lossless CST with typed navigation. It remains Wasm-clean, classifies Documenter
syntax without resolving it, and never invokes Julia at runtime. Julia's
Markdown stdlib is only a test-time differential oracle; regenerate its pinned
projections with `scripts/update-markdown-corpus.sh`.
...
Regenerate `fatou.schema.json` with
`UPDATE_EXPECTED=1 cargo test config_schema`, review the diff, and run
`cargo test config_schema`; the schema gate is generated from the actual raw
configuration types and validates representative accepted and rejected input.
