...
while aaaaaaaaaa && bbbbbbbbbb && cccccccccc && dddddddddd && eeeeeeeeee && ffffffffff && ggg
    body_statement
end

if aaaaaaaaaa || bbbbbbbbbb || cccccccccc || dddddddddd || eeeeeeeeee || ffffffffff || gggggggg
    x
end
...
if q
    a
elseif aaaaaaaaaa && bbbbbbbbbb && cccccccccc && dddddddddd && eeeeeeeeee && ffffffffff && gg
    b
end

# a comparison condition breaks after the operator, continuation at +8
while aaaaaaaaaaaaaaaaaaaaaaaaa < bbbbbbbbbbbbbbbbbbbbbbbbb + ccccccccccccccccccccccccccccccccccc
    body
end

# a bracketed predicate call breaks inside its parens: args at +8, close at +4
if some_predicate_function(argument_one, argument_two, argument_three, argument_four, arg_five)
    body_statement
end
...
function f()
    if aaaaaaaaaa && bbbbbbbbbb && cccccccccc && dddddddddd && eeeeeeeeee && ffffffffff && ggg
        x
    end
end
