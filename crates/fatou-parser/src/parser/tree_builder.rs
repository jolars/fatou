...
fn push_token(builder: &mut GreenNodeBuilder<'_>, tok: &Token<'_>) {
    // The lexer already materialized the token text as a `Cow<str>` — borrowed
    // within a chunk, owned across a boundary — so it hands the builder a plain
    // `&str` either way.
    builder.token(syntax_kind_for(tok.kind).into(), tok.text.as_ref());
}
