...
## Latest session (DIRECTION CHANGE: full reflow — Runic is not width-sensitive)

**Decision (user's call): Fatou fully reflows; line breaking is a wholesale
divergence from Runic.** Runic is *not* line-width-sensitive — it preserves the
user's line breaks and normalizes only spacing/indent within whatever layout the
source chose. Tenet 1 (now strengthened to general determinism) says the opposite:
same tokens => same output, breaking *only* where `line_width` or semantics
require it, never because the source broke. So `format(input) == runic(input)` is
**not** the target for any construct whose layout depends on a source break.
Docs landed: AGENTS.md Tenet 1 strengthened + retargeted to Runic (this restores
the accidentally-reverted `15a08a5`); `runic-blocked.txt` gained a FOUNDATIONAL
DIVERGENCE preamble.

**This obsoletes much of the landed machinery's *purpose*.** `lower_multiline_bracket`,
`lower_matrix` blanks, `binary_group_breaks` continuation indent, `lower_ternary`
continuation, `Ir::BlankLine` — all exist to *mirror* source breaks. They are not
wrong code, but they are no longer the parity target; they survive only as the
"false parity" that keeps current fixtures green. Reflow engine does not exist yet
(`line_width` is **vestigial** — a 110-char call comes back untouched).

### Audit of expected.jl (false parity under full reflow)

Three source-dependence axes found; all currently allowlisted ("false parity"),
all must become deliberate divergences (or Fatou-authored `expected.jl`) once
reflow lands. **Do not rip the fixtures out now** — reflow doesn't exist, the
coverage gate (`runic_corpus_fully_triaged`) needs every slug triaged, and
`expected.jl` is Runic-generated by policy.

- **A. Collapse-eligible source breaks in expressions** — expected keeps user
  breaks though each collapses to one line < 92. Full reflow collapses them.
  Fixtures: `binary_continuation`, `ternary_multiline`, `multiline_brackets`,
  `paren_multiline`, `ternary_paren_branch`.
- **B. Blank lines as content** — Tenet 1: whitespace never influences output, so
  interior blanks get stripped. Fixtures: `bracket_blank_lines`,
  `bracket_gap_blank_lines`, `matrix_blank_lines`, `matrix_gap_blank_lines`,
  `paren_blank_line_divergence` (already blocked — its rationale is now the
  *general* rule, not a lone-paren special case), plus leading-blank-in-body cases
  in `function_blocks`/`let_blocks`. **DECIDED (user):** blank lines *between
  statements* (top-level and block bodies) are kept as a **single** separator —
  collapse 2+ to 1, strip leading/trailing, strip *interior* blanks within an
  expression/bracket/matrix. So the rule is "keep-1 between statements, strip
  everywhere else," not strip-all.
- **C. `;` vs newline statement separator** — `a; b` ≡ `a` newline `b`; must format
  identically. Currently preserved distinct (`y = 2; z = 3` kept inline in
  `let_blocks`; `b; c` inline in `loop_blocks`). Diverges.
- **Not a divergence (semantic, keep):** matrix *row* breaks — newline is the row
  separator inside `[...]`, so `[1 2\n3 4]` cannot collapse (`matrices`,
  `multiline_matrices`, `matrix_comments` row structure is legitimate). Block
  bodies expanding to multiline is semantic, not break-preservation.

### Ranked next targets (reframed for full reflow)

1. **Build the width-driven layout engine** — the prerequisite for everything.
   `line_width` must actually drive breaking: collapse to one line when it fits,
   break (and indent) when it doesn't. This replaces source-break mirroring as the
   break decision. Largest piece of work in the formatter's life.
2. **Apply the decided axis-B blank-line policy** (keep-1 between statements; strip
   interior blanks): make `Ir::BlankLine` cap statement gaps at one blank and drop
   interior blanks uniformly across brackets/matrices/blocks/parens — removing the
   current per-construct inconsistency.
3. **Normalize `;`-separated statements** (axis C) to the canonical separator so
   `a; b` and newline-separated format identically.

(Superseded break-preservation targets — comments in a broken paren, multi-line
paren operand, ternary-in-condition — are recorded under Earlier sessions; they
were sharpening source-break mirroring and are off-mission now.)

## Earlier sessions

- **strip blank lines inside a broken paren (`paren_blank_line_divergence`)**: made
  Fatou strip interior blanks in a lone parenthesized value where Runic preserves
  them (dropped `lower_paren`'s >1-newline transparent bail). Recorded as a blocked
  divergence; superseded above as a special case of the general axis-B rule. Its
  three then-pending targets (comments in a broken paren; multi-line paren operand
  `y = (a +\nb) + c`; ternary-in-condition `(b ? c :\n d) ? x : y`) were all
  break-preservation refinements, now off-mission under full reflow.
...
- **n-ary binary + continuation indent (`lower_binary` generalized +
  ...
  to the full flat operand/operator alternation (so tight chains `a+b+c+d` finally
  normalize), plus a multi-line continuation indent mirroring `lower_ternary` —
  one shared `+4` level owned by the outermost group root (`is_group_root`) and
  gated on a real group break (`binary_group_breaks`, not a break buried in a
  bracket). Fixture `binary_continuation/`. Corpus 56→57.
