...
`losslessness`, `format-error`, `timeout`, `unknown`) filed as GitHub issues. This skill is
...
- **JuliaSyntax differential oracle** — fatou's parser is measured against
  ...
  needs Julia), so the test needs no Julia install. Run it with `cargo test -p
  fatou-parser --test juliasyntax_oracle` (see
...
5. **Triage (the heart of the work).** For each priority rule, pull real findings
   ...
   **reduce each suspect to a minimal reproducer**. `lint` takes a path, not
   stdin, so write the snippet to a scratchpad file:
