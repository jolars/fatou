...
## Latest session (chained postfix tail on a breaking call — gate the continuation-aware break)

Continued the ranked-#1 trailing-tail sweep. After last session's single postfix tail, gated the
**chained** case: multiple postfix ops after a wide call's closing bracket. The continuation-aware
`fits` (2026-07-02b) already rides the whole tail on the `)` line — `).field.other`,
`)[index_expr][second]`, `).method(z)` (dot-then-call), `)[idx].field` (index-then-dot). **No code
change** — pure `test(formatter)` gating of already-canonical output. Verified idempotence and
input-independence (pre-broken source reflows to the identical form). User chose (AskUserQuestion):
**chained-postfix-on-call** over postfix-on-a-breaking-collection (deferred). Gated
`chained_postfix_break/` (four lines). Gate 80→81. Full suite + clippy + `fmt --check` clean.

**Also probed, deferred:** a postfix tail on a breaking **collection** works cleanly for `.field`
and `::T` (`[a, …,]::VectorType`), but `<wide-collection>[index]` breaks the *index* arg-list
instead of the collection (`[a, …][\n    idx,\n]`) — a questionable but engine-consistent layout;
revisit as its own target if we want the collection to break instead.

**Parser/lexer blocker surfaced:** none.

**Ranked next targets:** (1) keep sweeping now-correct **trailing-tail** breaks — a postfix tail on
a breaking **collection**/**matrix** (`[a, b, …].field`, `::T` work; the `[index]`-on-collection
subject-vs-index break above needs a decision), or a call that breaks *inside* an argument tail.
(2) The arrow/pair tier (`=> --> <->`) could flatten across kinds like the additive/multiplicative
tiers but is rare — revisit only on a real case. (3) Sweep the residual Runic doc comments in
`rules.rs` (`is_tight_binop`'s doc still cites `runic-blocked.txt`).
