...
## Latest session (chained-pair hug through the whole spine)

Closed ranked next-target #1 (`feat(formatter)`). A trailing chained pair
`a => b => Dict(...)` now hugs its enclosing bracket exactly like a single pair —
the whole `a => b => ` joins the flat hug prefix and only the innermost construct
explodes — instead of bailing to normal arrow-tier layout (`foo(` exploding,
each `=>` breaking). User chose this (AskUserQuestion **Option A**, hug the whole
chain) over keeping the bail.

**Change:** split `pair_hug_split` into `pair_operands` (the clean two-operand
`=>`/`.=>` parse, no huggable check) + the **recursive** `pair_hug_chain`, which
peels the right-nested spine — accumulating `a => b => ` into one prefix `Ir` and
returning the innermost huggable construct node. The four consumers
(`value_is_huggable`, `pair_hug_grouped_parts` for the grouped arg/kwarg/element
sites, `hug_value_parts` for the ungrouped index-subject sites) all route through
it; no IR/printer change. A single pair is the depth-1 base case, byte-identical
to before. Bails preserved: a `-->`/`<-->` link (not the pair idiom — `pair_operands`
returns `None`) and a non-huggable innermost value both fall back to normal layout.

Gated `chained_pair_hug/` (9 cases: hug at arg/deep/element/kwarg sites, flat-fit
stays flat, source-broken reflows to the hug, `plain` non-huggable-leaf bail,
`arrowtier` `-->` bail). Updated the `pair_hug/` fixture's `chained` case (it
locked the now-obsolete bail — a conscious, user-approved divergence). Gate 96→97,
stability + clippy + fmt green. No parser/lexer blocker surfaced.
...
are already width-driven. (2) Bare tuple with an interior newline still bails
transparent (`lower_bare_tuple`, the reflow debt) — no fixture input exercises it;
author one and drive it width-first.
