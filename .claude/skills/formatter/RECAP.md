...
## Latest session (widen arrow_pair_chain for `<--`/`.<--`)

Closed ranked next-target #3 (fixtures-only, `test(formatter)`, **no code
change**). The lexer gap that kept `<--` out of the fixture is resolved
(`TODO.md` Parser `[x]`): `LEFT_LONG_ARROW`/`DOT_LEFT_LONG_ARROW` already sit in
`binary_prec_class` tier 3 (`rules.rs:341`), so a `<--`/`.<--` chain breaks
uniformly on the arrow tier exactly like `=>`/`-->`/`<-->` — but no fixture
exercised it, so a tier-list regression would go uncaught.

**Change:** widened `arrow_pair_chain/` with four `<--` cases — `s` (same-op
`<--` chain, breaks at every op), `t` (mixed `<--`/`.<--`/`=>`, breaks
uniformly), `u` (short `<--` stays flat), `p` (source-broken `<--` reflows flat,
locking input-independence). Verified each is the canonical fully-reflowed form
(the code path is the same one `=>`/`-->`/`<-->` already use). Gate green,
stability (idempotence + clean reparse) green, clippy + fmt clean. Gate count
unchanged at 96 (same fixture, wider). No parser/lexer blocker surfaced.
...
debt #1); pick one and drive it width-first.
