...
keyword-shaped table (`KEYWORDS`, `keyword_kind`, `TokKind::is_keyword`, the
`syntax_kind_for` mapping, `SyntaxKind::is_keyword`) is generated by expanding
`keyword_table!` with a local generator macro, so they cannot drift.
