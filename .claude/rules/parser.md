...
**A new token is a new `tokens.rs` row**, never a new enum variant: the
`TokKind` variant, the `SyntaxKind` variant, and the `syntax_kind_for` mapping
all come from expanding `token_table!` with a local generator macro, so a token
cannot be lexed but unmapped. The rows are 1:1; the tokens that are not (the six
Unicode `call-i` tiers, all one `UNICODE_OP`) are declared literally beside each
generator, and `syntax_kind_for`'s exhaustive match is what forces them to be
mapped.

**A new keyword is a new `keywords.rs` row**, never a new arm: every
keyword-shaped table (`KEYWORDS`, `keyword_kind`, `TokKind::is_keyword`,
`SyntaxKind::is_keyword`) is generated by expanding `keyword_table!`, whose rows
are also spliced into `token_table!` — so one row still gets the keyword its two
enum variants and its mapping.
