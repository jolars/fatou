...
import { resolveFatouBinary } from "./installer";
...
async function resolveCommandPath(
	...
	const githubRepo = config.get<string>("githubRepo", "jolars/fatou");
	...
	if (strategy === "path") {
		...
		if (!executablePath || executablePath.trim().length === 0) {
			void vscode.window.showWarningMessage(
				"fatou.executableStrategy is set to 'path' but fatou.executablePath is empty. Falling back to 'fatou' on PATH.",
			);
			return "fatou";
		}
	...
	if (!versionPinExplicit) {
		...
		if (bundled) {
			outputChannel.appendLine(`Using bundled Fatou binary at ${bundled}.`);
			return bundled;
		}
	}
	...
	if (nixOs) {
		outputChannel.appendLine(
			"Detected NixOS; skipping binary download and using 'fatou' on PATH.",
		);
		return "fatou";
	}

	try {
		return await resolveFatouBinary(
		...
		void vscode.window.showWarningMessage(
			`Fatou binary download failed (${message}). Falling back to 'fatou' on PATH.`,
		);
		return "fatou";
	}
}
...
/** Whether a `fatou.<section>.enable` toggle is on (default on). Read live so
...
function featureEnabled(
	...
	return vscode.workspace
		.getConfiguration("fatou")
		.get<boolean>(`${section}.enable`, true);
}

/**
 * Client-side gates for the three `fatou.*.enable` toggles. Each request is
 * intercepted before it reaches (or after it leaves) the server: diagnostics can
 * be dropped wholesale (including the parse errors a `fatou.toml` cannot mute)
...
async function startClient(
	...
	const config = vscode.workspace.getConfiguration("fatou");
	...
	try {
		...
		outputChannel.appendLine(
			`Failed to start Fatou language server: ${message}`,
		);
		void vscode.window.showErrorMessage(
			`Fatou language server failed to start: ${message}`,
		);
		client = undefined;
		return;
	}
...
async function restartClient(
	...
	if (client) {
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			outputChannel.appendLine(
				`Error stopping Fatou language server: ${message}`,
			);
		}
		client = undefined;
	}
...
export async function activate(
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("fatou.restart", () =>
			restartClient(context, outputChannel),
		),
	);

	// The middleware reads the toggles live, so formatting/languageFeatures need
	// no restart. Diagnostics may be pushed, though, so toggling them must flush
	// the already-published set: clear on disable, restart to repopulate on
	// enable.
	context.subscriptions.push(
		vscode.workspace.onDidChangeConfiguration((event) => {
			if (!event.affectsConfiguration("fatou.diagnostics")) {
				return;
			}
			if (featureEnabled("diagnostics")) {
