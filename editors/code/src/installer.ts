...
function detectTargetAsset(): TargetAsset {
	...
	if (process.platform === "darwin" && process.arch === "arm64") {
		return { archiveNames: ["fatou-aarch64-apple-darwin.tar.gz"], binaryName };
	}
	if (process.platform === "darwin" && process.arch === "x64") {
		return { archiveNames: ["fatou-x86_64-apple-darwin.tar.gz"], binaryName };
	}
	if (process.platform === "linux" && process.arch === "arm64") {
		return {
			archiveNames: [
				"fatou-aarch64-unknown-linux-gnu.tar.gz",
				"fatou-aarch64-unknown-linux-musl.tar.gz",
			],
			binaryName,
		};
	}
	if (process.platform === "linux" && process.arch === "x64") {
		return {
			archiveNames: [
				"fatou-x86_64-unknown-linux-gnu.tar.gz",
				"fatou-x86_64-unknown-linux-musl.tar.gz",
			],
			binaryName,
		};
	}
	if (process.platform === "win32" && process.arch === "x64") {
		return { archiveNames: ["fatou-x86_64-pc-windows-msvc.zip"], binaryName };
	}
	if (process.platform === "win32" && process.arch === "arm64") {
		return { archiveNames: ["fatou-aarch64-pc-windows-msvc.zip"], binaryName };
	}
...
export async function resolveFatouBinary(
		...
		try {
			selectedRelease = await withRetries(async () => {
				...
				if (candidate.kind === "latest-list") {
					...
					throw new Error(
						`No stable Fatou release asset '${target.archiveNames.join("' or '")}' found for ${repo}@latest`,
					);
				}
				...
				if (
					...
					throw new Error(
						`${repo}/releases/latest points at '${release.tag_name}', which is not a Fatou release`,
					);
				}
