...
//!
//! Only the unambiguous shape is matched: `occursin` at its two-argument form,
//! with the callee confirmed to be Base's, and a plain `r"..."` pattern — no
//! flag suffix, since `i` and `m` both change what an anchor means — anchored
//! at exactly one end with a metacharacter-free remainder. `r"^abc$"` is an
...
use crate::ast::{AstNode, Expr, StringLiteral};
use crate::linter::diagnostic::{Applicability, Diagnostic, Fix};
use crate::linter::rules::regex::Anchor;
...
impl Rule for StringBoundary {
    ...
    fn description(&self) -> &'static str {
        "Flag `occursin(r\"^abc\", s)` and `occursin(r\"abc$\", s)`, boundary \
         ...
         match rather than a boundary test. `occursin` must be confirmed to be \
         Base's.\n\n\
    ...
    fn check(&self, el: &SyntaxElement, ctx: &RuleContext<'_>, sink: &mut Vec<Diagnostic>) {
        ...
        let (Some(Expr::StringLiteral(literal)), Some(haystack)) = (args.first(), args.get(1))
        else {
            return;
        };
        ...
        let Some((anchor, rest)) = regex::single_anchor(&pattern) else {
            return;
        };
        ...
        let message = match (
            ...
            rewrite::inline_text(haystack.syntax()),
            rewrite::inline_text(literal.syntax()),
        ) {
            (Some(new_literal), Some(haystack), Some(old)) => format!(
                "test `{func}({haystack}, {new_literal})` instead of matching \
                 the anchored regex `{old}`"
            ),
        ...
        if let Some(fix) = boundary_rewrite(ctx, node, literal, haystack, &boundary) {
            diag.fixes.push(fix);
        }
        sink.push(diag);
    }
...
/// The fix replacing the whole `occursin` call with the boundary predicate:
/// the haystack's own source text, and the rebuilt pattern literal.
///
...
/// the replaced span carries a comment outside the two reused pieces — which
