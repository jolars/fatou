...
weekly automated corpus scan's *formatter* regressions (`idempotency`,
`losslessness`, `format-error`, `timeout`, `unknown`) filed as GitHub issues.
This skill is proactive and interactive: you choose a repo and go looking for
linter/parser quality problems. Formatter losslessness and idempotence are out
of scope here—leave them to `smoke-test-triage`.
...
- **JuliaSyntax differential oracle** — fatou's parser is measured against
  JuliaSyntax.jl, the official Julia parser. There is no Rust dependency on it:
  the expected s-expressions are **pinned as committed `expected.sexpr` files**
  (regenerated by `scripts/update-juliasyntax-corpus.sh`, the one task that
  needs Julia), so the test needs no Julia install. Run it with
  `cargo test -p   fatou-parser --test juliasyntax_oracle` (see
  `crates/fatou-parser/tests/juliasyntax_oracle.rs` and the
  `crates/fatou-parser/tests/fixtures/oracle/` corpus). This is the ground truth
  for parser outcomes on covered cases.
- **fatou's own losslessness** — `fatou parse --verify --quiet <file>` proves
  ...
  shape. Note **`lint` takes a path argument only — no stdin** (`parse` and
  `format` do accept `-`/piped stdin), so write reproducers to a scratchpad file
  first and drive everything from a path.
...
5. **Triage (the heart of the work).** For each priority rule, pull real
   ...
   line, and **reduce each suspect to a minimal reproducer**. `lint` takes a
   path, not stdin, so write the snippet to a scratchpad file:
   ...
   Rule selection is config-driven (`[lint] select`/`ignore` in a `fatou.toml`),
   **not** a `--select` CLI flag — `lint`'s only flags are `--fix`,
   `--unsafe-fixes`, `--exclude`, `--force-exclude`, `--julia-version`, and
   `--output`. To count findings, count `warning:`/`-->` lines, not token
...
7. **Fan out for volume (recommended).** For a big finding set, spawn parallel
   ...
   **Subagent findings are leads, not verdicts.** They reliably surface
   *classes* and counts, but re-verify each one yourself before acting—a
   plausible "it's used here" can be a use inside a docstring code fence (not
   real code), an inflated cross-file estimate from a whole-word grep, or a
   miscounted bucket. Reduce it, run the oracle, then believe it.
...
8. **Fix or record.** For the cleanest, well-isolated bugs, fix TDD-style,
   ...
   - **Pick the right layer.** "Fix at the root cause, never paper over
     ...
     scope-transparent wrapper like `@inbounds`/`@testset` are indistinguishable
     without expanding the macro). There, prefer the **narrowest layer that
     removes the FP without introducing a false negative elsewhere**: a
     conservative rule-level guard (as `redefined-constant` got for
     macro-argument sites) can be more correct than a blanket semantic-model
     change that would wrongly suppress the transparent case.
   - Run the gates: `cargo test --workspace`,
     `cargo clippy --workspace --all-targets --all-features --      -D warnings`,
     `cargo fmt --all -- --check`; `cargo insta accept` after reviewing new
     snapshots.

   Record everything you don't fix as follow-ups in `TODO.md` in the house
   style, each with a minimal reproducer and the confirmed-correct Julia
   behavior. Commit only if the user asks—atomic, Conventional Commits.

9. **Report back.** State plainly: bugs found (fixed vs. documented) with
   copy-pasteable reproducers; false-positive categories per rule;
   incorrect-span issues; which rules you verified clean; and the follow-ups
   recorded. Be faithful about what was and wasn't oracle-verified—especially
   where `julia` was unavailable and you reasoned instead.

## fatou-specific notes

- **`fatou`'s parser is an architectural twin of arity's** (lossless rowan CST,
  salsa). Expect the same *classes* of parser bugs (trivia/continuation
  handling, bracket-context newline rules), adapted to Julia's grammar.
...
- **Autofix correctness is correctness, not layout.** A fix must keep the code
  ...
  formatter's job). Test a fix by applying it and re-parsing, never by reading
  it.
